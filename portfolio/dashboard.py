...
def render_portfolio_dashboard(*, embedded: bool = False):
    ...
    with tab_returns:
        ...
        st.dataframe(rets["summary"], hide_index=True, use_container_width=True)

        c_risk, c_ch = st.columns(2)
        with c_risk:
            st.markdown("##### Risk-Adjusted Returns")
            st.dataframe(rets["risk_adjusted"], hide_index=True, use_container_width=True)
        with c_ch:
            st.markdown("##### Charges Impact")
            st.dataframe(rets["charges"], hide_index=True, use_container_width=True)
        ...
        st.dataframe(rets["monthly_compare"], hide_index=True, use_container_width=True)
    ...
    with tab_year:
        ...
        if ydf.empty:
            ...
            with col_pl:
                ...
                st.plotly_chart(fig_ypl, use_container_width=True)
    ...
    with tab_regime:
        ...
        st.dataframe(vix_show, hide_index=True, use_container_width=True)
